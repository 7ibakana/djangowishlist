...
def place_list(request):
    ...
    and redirect to this same page. This creates a GET to this same route.

    If not a POST route, or Place is not valid,
    display a page with a list of places and a form to add a new place. 
    """

    if request.method == 'POST':
        form = NewPlaceForm(request.POST)
        place = form.save() # Create a new Place from the form
        if form.is_valid(): # Checks against DB contraints, for example fields present?
