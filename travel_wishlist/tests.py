...
# tests the homepage, the wishlist, the visted list and if a new place is added to the wishlist
...
class TestNoVisitedPlace(TestCase):
...
class TestVisitPlace(TestCase):
    ...
    def test_visit_non_existent_place(self):
        ...
        response = self.client.post(visit_place_url, follow=True)
        self.assertEqual(404, response.status_code) # n ot found